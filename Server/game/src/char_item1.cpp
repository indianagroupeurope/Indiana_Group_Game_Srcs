...
bool CHARACTER::DoRefineWithScroll(LPITEM item)
{
	...
	else if (pkItemScroll->GetValue(0) == SOULP_SCROLL)
	{
		if (item->GetLevelLimit() > pkItemScroll->GetValue(1))
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can not use soul scroll this item."));
			return false;
		}
	}
...
void CHARACTER::__OpenOfflineShop()
{
	...
	if (GetLevel() < g_bMinLevel)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ct_icin_yeterli_seviyede_degilsin"), g_bMinLevel);
		return;
	}
	...
	if (IsAttrTransferOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("kostum_bonus_transfer_pencersi_acik"));
		return;
	}
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	if (item->GetVnum() == 55009)    {
		int number1461 = number(55010,55027);//item kodları
	...
	if (item->GetVnum() == 55001) {
		...
		if (pmsg2->Get()->uiNumRows > 0) {
			if (GetNewPetSystem()->IsActivePet()){ChatPacket(CHAT_TYPE_INFO,LC_TEXT("PSS_PETI_GONDER"));return false;}
			MYSQL_ROW row = mysql_fetch_row(pmsg2->Get()->pSQLResult);
			int suankiDuration=atoi(row[1])-atoi(row[0]);
			//ChatPacket(CHAT_TYPE_INFO,"%d",suankiDuration);
			if(suankiDuration<1){
		...
		case ITEM_CAMPFIRE:
			{
				...
				if (GetMapIndex() == 113)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BU_MAPDA_ATES_YAKAMASSIN"));
					return false;
				}

				int son_ates = GetQuestFlag("kamp.ates");
				if (get_global_time() - son_ates < 60)
				{
					ChatPacket(CHAT_TYPE_INFO, LC_TEXT("60_SANIYE_BEKLE"));
					return false;
				}
				...
				switch (item->GetSubType())
				{
					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case 27124: // New Item Bandage for Bleed
								RemoveBleeding();
								break;
							...
							case 50299: // cevrimdisi tezgah
								...
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't open offline shop in this place!"));
								}
								break;
							...
							case 50107:
								{
									if (CArenaManager::instance().IsArenaMap(GetMapIndex()) == true)
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("´e·A Aß¿¡´A AI¿eCO ¼o ¾ø´A ¹°C°AO´I´U."));
										return false;
									}
					...
					case USE_ABILITY_UP:
						{
							...
							{
								case APPLY_MOV_SPEED:
									if (FindAffect(AFFECT_MOV_SPEED)) // @fixme61
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BU_ETKI_DEVAM_EDIYOR"));
										return false;
									}
								...
								case APPLY_ATT_SPEED:
									if (FindAffect(AFFECT_ATT_SPEED)) // @fixme61
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BU_ETKI_DEVAM_EDIYOR"));
										return false;
									}
								...
								case USE_COSTUME_ENCHANT:
									{
										...
										item2->ChangeAttribute(aiChangeProb);
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("efsun_basariyla_degistirildi"));
										{
											char buf[21];
											snprintf(buf, sizeof(buf), "%u", item2->GetID());
								...
								case USE_COSTUME_TRANSFORM:
									{
										...
										int aiChangeProb[ITEM_ATTRIBUTE_MAX_LEVEL] = {100, 100, 100, 100, 100};
										item2->ChangeAttribute(aiChangeProb);
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("efsun_basariyla_aktarildi"));
										{
											char buf[21];
											snprintf(buf, sizeof(buf), "%u", item2->GetID());
...
bool CHARACTER::UseItem(TItemPos Cell, TItemPos DestCell)
{
	...
	if (item->GetVnum() == MASTER_RESIST_BLESS_ITEM_BUFF_VNUM)
	{
		if (GetOfflineShopOwner() || GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen() || IsAttrTransferOpen())
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't use buff items while you have exchange,safebox,refine or other windows opened!"));
			return false;
		}
		if (true == IsDead())
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't use buff items while you're dead!"));
			return false;
		}	
		else
		{
		...
		{	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Resistance Buff, [Item Buff]!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));				
			return false;
		}	
		if (IsAffectFlag(AFF_HOSIN))
		{	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Resistance Buff, [Shaman Buff]!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));				
			return false;
		}		
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Resistance Buff , [Item Buff] for 160 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff resist rate applied: 24%."));			
		}			
	}
}
	...
	if (item->GetVnum() == GRAND_RESIST_BLESS_ITEM_BUFF_VNUM) // 
	{
		...
		}	
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Resistance Buff , [Item Buff] for 224 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff resist rate applied: 29%."));			
		}			
	}
}
	...
	if (item->GetVnum() == PERFECT_RESIST_BLESS_ITEM_BUFF_VNUM) // 
	{
		...
		}		
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Resistance Buff, [Item Buff] for 310 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff resist rate applied: 35%."));			
		}			
	}
}
	...
	if (item->GetVnum() == MASTER_CRITICAL_BLESS_ITEM_BUFF_VNUM)
	{
		...
		{	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Dragon's Strength(Critical Buff), [Item Buff]!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));				
			return false;
		}	
		if (IsAffectFlag(AFF_GICHEON))
		{	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Dragon's Strength(Critical Buff), [Shaman Buff]!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));				
			return false;
		}			
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Dragon's Strength(Critical Buff), [Item Buff] for 110 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff critical rate applied: 24%."));			
		}			
	}
}
	...
	if (item->GetVnum() == GRAND_CRITICAL_BLESS_ITEM_BUFF_VNUM) // 
	{
		...
		}	
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Dragon's Strength(Critical Buff), [Item Buff] for 142 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff critical rate applied: 29%."));			
		}			
	}
}
	...
	if (item->GetVnum() == PERFECT_CRITICAL_BLESS_ITEM_BUFF_VNUM) // 
	{
		...
		if (true == IsDead())
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Dragon's Strength(Critical Buff)!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));	
			return false;
		}	
		else
		...
		}	
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Dragon's Strength(Critical Buff), [Item Buff] for 185 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff critical rate applied: 35%."));			
		}			
	}
}
	...
	if (item->GetVnum() == MASTER_REFLECT_BLESS_ITEM_BUFF_VNUM)
	{
		...
		{	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Reflection, [Item Buff]!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));				
			return false;
		}
		if (IsAffectFlag(AFF_BOHO))
		{	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You're already blessed with: Reflection, [Shaman Buff]!"));	
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Wait for your current buff to disappear and try again!"));				
			return false;
		}		
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Reflection, [Item Buff] for 160 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff reflection rate applied: 21%."));			
		}			
	}
}
	...
	if (item->GetVnum() == GRAND_REFLECT_BLESS_ITEM_BUFF_VNUM) // 
	{
		...
		}
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Reflection, [Item Buff] for 224 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff reflection rate applied: 31%."));			
		}			
	}
}
	...
	if (item->GetVnum() == PERFECT_REFLECT_BLESS_ITEM_BUFF_VNUM) // 
	{
		...
		}
		{
			...
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You are blessed now with: Reflection, [Item Buff] for 310 seconds."));
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Buff reflection rate applied: 45%."));			
		}			
	}
}
#endif
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
	...
	if (!CanHandleItem())
	{
		if (NULL != DragonSoul_RefineWindow_GetOpener())
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PENCERE_ACIKK."));
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ruha_bagli_nesneyi_yere_dusuremezsin"));
		return false;
	}
	...
	// It looks the player dropped min. 5 items in the past 1 second
	if (thecore_pulse() < LastDropTime + 25 && CountDrops >= 5)
	{
		// Set it to 0
		CountDrops = 0;
...
bool CHARACTER::SellItem(TItemPos Cell)
{
	...
	if (item->IsBind() || item->IsUntilBind())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("RUHA_BAGLI_NESNELER_SATILAMAZ"));
		return false;
	}

	if (IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_SELL))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BU_NESNE_SATILAMAZ"));
		return false;
	}
...
bool CHARACTER::DropGold(int gold)
{
	...
	if (item)
	{
		...
		if (item->AddToGround(GetMapIndex(), pos))
		{
			...
			if (gold > 1000) // Aμ¿ø AI≫o¸¸ ±a·ICN´U.
				LogManager::instance().CharLog(this, gold, "DROP_GOLD", "");

			item->StartDestroyEvent(5);
		}
...
bool CHARACTER::SwapItem(BYTE bCell, BYTE bDestCell)
{
	...
	if (TItemPos(EQUIPMENT, item2->GetCell()).IsEquipPosition())
	{
		...
		if (item2->IsDragonSoul()) // @fixme117
		{
			if (false == CanUnequipNow(item2) || false == CanEquipNow(item1))
				return false;
		}
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("bu_islemi_bukadar_hizli_yapamazsin"));
		return false;
	}
	...
	if (true == item->IsEquipped())
	{
		...
		{
			SetQuestFlag("zirh.engel", get_global_time() + 2);
		}
	}
...
bool CHARACTER::CanReceiveItem(LPCHARACTER from, LPITEM item) const
{
	...
	switch (GetRaceNum())
	{
		...
		case ALCHEMIST_MOB:
			...
			if (item->IsBind() || item->IsUntilBind())
			{
				from->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("KILIT_SIMYA"));
				return false;
			}
#endif
