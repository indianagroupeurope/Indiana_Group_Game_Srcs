...
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT player_name, player_empire, kill_count FROM log.dragonlair_dungeon%s WHERE player_id=%u;", get_table_postfix(), dwPlayerID));
...
void LogManager::WhisperLog(const char * from, const char * to, const char * message)
{
	...
	Query("INSERT INTO whisper_log (kimden, kime, mesaj) VALUES ('%s', '%s', '%s')", from, to, __escape_hint);
}
#endif
...
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT partecipants, time FROM log.meley_dungeon%s WHERE guild_id=%u;", get_table_postfix(), dwGuildID));
	if (pMsg->Get()->uiNumRows == 0)
