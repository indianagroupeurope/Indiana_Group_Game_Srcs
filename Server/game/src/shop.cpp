...
bool CShop::Create(DWORD dwVnum, DWORD dwNPCVnum, TShopItemTable * pTable)
{
	...
	for (bItemCount = 0; bItemCount < SHOP_HOST_ITEM_MAX_NUM; ++bItemCount){if (0 == (pTable + bItemCount)->vnum){break;}}
...
{
	if (bItemCount > SHOP_HOST_ITEM_MAX_NUM){return;}
	...
	for (int i = 0; i < bItemCount; ++i)
	{
		...
		if (m_pkPC)
		{
			...
			if (!pkItem){sys_err("cannot find item on pos (%d, %d) (name: %s)", pTable->pos.window_type, pTable->pos.cell, m_pkPC->GetName());continue;}
			item_table = pkItem->GetProto();
		}
		else
		{
			if (!pTable->vnum){continue;}
			item_table = ITEM_MANAGER::instance().GetTable(pTable->vnum);
		}

		if (!item_table){sys_err("Shop: no item table by item vnum #%d", pTable->vnum);continue;}
		...
		else{iPos = m_pGrid->FindBlank(1, item_table->bSize);}
		...
		else
		{
			...
			{
				if (item_table->dwGold == 0){item.price = item.count;}
				else{item.price = item.count / item_table->dwGold;}
			}
			else{item.price = item_table->dwGold * item.count;}
		}
...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	if (m_pkPC){item = r_item.pkItem;}
	else{item = ITEM_MANAGER::instance().CreateItem(r_item.vnum, r_item.count);}

	if (!item){return SHOP_SUBHEADER_GC_SOLD_OUT;}

	int iEmptyPos;
	if (item->IsDragonSoul()){iEmptyPos = ch->GetEmptyDragonSoulInventory(item);}
	else{iEmptyPos = ch->GetEmptyInventory(item->GetSize());}
	...
	if (m_pkPC)
	{
		...
		if (iVal > 0){m_pkPC->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("판매금액의 %d %% 가 세금으로 나가게됩니다"), iVal);}
	...
	else
	{
		...
		if (item->GetVnum() >= 80003 && item->GetVnum() <= 80007){LogManager::instance().GoldBarLog(ch->GetPlayerID(), item->GetID(), PERSONAL_SHOP_BUY, "");}

		DBManager::instance().SendMoneyLog(MONEY_LOG_SHOP, item->GetVnum(), -dwPrice);
	}

	if (item){sys_log(0, "SHOP: BUY: name %s %s(x %d):%u price %u", ch->GetName(), item->GetName(), item->GetCount(), item->GetID(), dwPrice);}
	ch->Save();
...
{
	if (!ch){return false;}
	if (ch->GetExchange()){return false;}
	if (ch->GetShop()){return false;}
	...
	for (DWORD i = 0; i < m_itemVector.size() && i < SHOP_HOST_ITEM_MAX_NUM; ++i)
	{
		...
		if (m_pkPC && !item.pkItem){continue;}
...
void CShop::Broadcast(const void * data, int bytes)
{
	...
	{
		LPCHARACTER ch = it->first;
		if (ch->GetDesc()){ch->GetDesc()->Packet(data, bytes);}
		++it;
	}
}
...
void CShop::BroadcastUpdateItem(BYTE pos)
{
	...
	if (m_pkPC && !m_itemVector[pos].pkItem){pack2.item.vnum = 0;}
	else
	{
