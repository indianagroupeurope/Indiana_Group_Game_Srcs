		...
		if (item->IsDragonSoul())
		{
			...
			else
			{
				...
				if (!ch->DragonSoul_IsQualified())
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, "Envanter dolu.");
			}
		}
		else
		{
...
#ifdef NEW_PET_SYSTEM
		if (!pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 && pkChr->IsNPC() && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && pkChr->GetRider() == NULL)
#else
		if (!pkChr->IsOfflineShopNPC() && pkChr->GetRaceNum() != 30000 && ppkChr->IsNPC() && !pkChr->IsPet() && pkChr->GetRider() == NULL)
#endif
		{
...
ACMD(do_item_purge)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "PET_BLOCK");
		return ;
	}
#endif
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   FIRE:%3d%% ELEC:%3d%% MAGIC:%3d%% WIND:%3d%% CRIT:%3d%% PENE:%3d%% ANTI-MAGIC:%3d%%", tch->GetPoint(POINT_RESIST_FIRE), tch->GetPoint(POINT_RESIST_ELEC), tch->GetPoint(POINT_RESIST_MAGIC), tch->GetPoint(POINT_RESIST_WIND), tch->GetPoint(POINT_RESIST_CRITICAL), tch->GetPoint(POINT_RESIST_PENETRATE), tch->GetPoint(POINT_ANTI_RESIST_MAGIC));
#else
	ch->ChatPacket(CHAT_TYPE_INFO, "   FIRE:%3d%% ELEC:%3d%% MAGIC:%3d%% WIND:%3d%% CRIT:%3d%% PENE:%3d%%", tch->GetPoint(POINT_RESIST_FIRE), tch->GetPoint(POINT_RESIST_ELEC), tch->GetPoint(POINT_RESIST_MAGIC), tch->GetPoint(POINT_RESIST_WIND), tch->GetPoint(POINT_RESIST_CRITICAL), tch->GetPoint(POINT_RESIST_PENETRATE));
#endif
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   TREE:%3d%% MONSTER:%3d%%", tch->GetPoint(POINT_ATTBONUS_TREE), tch->GetPoint(POINT_ATTBONUS_MONSTER));
	ch->ChatPacket(CHAT_TYPE_INFO, "   WARR:%3d%% ASAS:%3d%% SURA:%3d%% SHAM:%3d%% WOLF:%3d%%", tch->GetPoint(POINT_ATTBONUS_WARRIOR), tch->GetPoint(POINT_ATTBONUS_ASSASSIN), tch->GetPoint(POINT_ATTBONUS_SURA), tch->GetPoint(POINT_ATTBONUS_SHAMAN), tch->GetPoint(POINT_ATTBONUS_WOLFMAN));
	...
	if (*arg2 && !strcmp(arg2, "offlineshop"))
	{
		...
		if (pMap)
		{
			...
			if (offlineShop.bResult)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s offline shop is destroyed!", arg1);
		}
	}
...
typedef enum do_set_types_s {GOLD, RACE, SEX, JOB, EXP, MAX_HP, MAX_SP, SKILL, ALIGNMENT, ALIGN, CHEQUE, YENI_BIRIM} do_set_types_t;
}
...
const struct set_struct 
{
	...
	{ "sex",		NUMBER,	"0. Erkek, 1. Kadin"	},
	{ "job",		NUMBER,	"0. Yok, 1. Birinci, 2. Ikinci"	},
	{ "exp",		NUMBER,	NULL	},
	{ "max_hp",		NUMBER,	NULL	},
	{ "max_sp",		NUMBER,	NULL	},
	{ "skill",		NUMBER,	NULL	},
...
ACMD(do_set)
{
	...
	if (!*arg1 || !*arg2 || !*arg3)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Kullanim: set <name> <field> <value>");
	...
	switch (i)
	{
		...
		case DoSetTypes::YENI_BIRIM:
			{
				int yeniBirim = 0;
				str_to_number(yeniBirim, arg3);
				if(yeniBirim + tch->yeniBirim_miktar() > YENIBIRIM_MAX)
					return;
				tch->yeniBirim_guncelle(tch->yeniBirim_miktar()+yeniBirim);
			}
			break;
	}
...
ACMD (do_item_full_set)
{
	...
	switch (job)
	{
		case JOB_WARRIOR:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(15459);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(14569);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(18009);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(71135);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//Kusak
			item = ITEM_MANAGER::instance().CreateItem(85024);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//Silah Kostumu
			item = ITEM_MANAGER::instance().CreateItem(40101);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//RuzgarAyakkabisi
		...
		case JOB_ASSASSIN:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(79504);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(71135);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//Silah Kostumu
			item = ITEM_MANAGER::instance().CreateItem(40102);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//RuzgarAyakkabisi
			item = ITEM_MANAGER::instance().CreateItem(72701);
		...
		case JOB_SHAMAN:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//Silah Kostumu
			item = ITEM_MANAGER::instance().CreateItem(40105);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//RuzgarAyakkabisi
			item = ITEM_MANAGER::instance().CreateItem(72701);
		...
		case JOB_WOLFMAN:
		{
			item = ITEM_MANAGER::instance().CreateItem(20959);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(13149);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(6099);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//Silah Kostumu
			item = ITEM_MANAGER::instance().CreateItem(40107);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			//RuzgarAyakkabisi
			item = ITEM_MANAGER::instance().CreateItem(72701);
...
ACMD (do_attr_full_set)
{
	...
	switch (job)
	{
		case JOB_WARRIOR:
		{
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_ATTBONUS_HUMAN, 15);
				item->SetForceAttribute(1, APPLY_CRITICAL_PCT, 15);
			...
			if (item != NULL)
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_ATTBONUS_HUMAN, 15);
				item->SetForceAttribute(1, APPLY_RESIST_MAGIC, 15);
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_DEX, 12);
				item->SetForceAttribute(1, APPLY_BLOCK, 15);
				item->SetForceAttribute(2, APPLY_ATTBONUS_HUMAN, 15);
			...
			if (item != NULL)
			{
				...
				item->SetForceAttribute(1, APPLY_ATTBONUS_HUMAN, 15);
				item->SetForceAttribute(2, APPLY_RESIST_MAGIC, 15);
				item->SetForceAttribute(3, APPLY_STEAL_HP, 15);
				item->SetForceAttribute(4, APPLY_PENETRATE_PCT, 15);
			}
			...
			item = ch->GetWear(WEAR_COSTUME_SASH);
			if (item != NULL)
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_ATTBONUS_HUMAN, 15);
				item->SetForceAttribute(1, APPLY_STR, 15);
				item->SetForceAttribute(2, APPLY_INT, 15);
				item->SetForceAttribute(3, APPLY_BLEEDING_PCT, 15);
				item->SetForceAttribute(4, APPLY_ATTBONUS_MONSTER, 50);
			}
			...
			if (item != NULL)
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_ATTBONUS_HUMAN, 15);
				item->SetForceAttribute(1, APPLY_STR, 50);
				item->SetForceAttribute(2, APPLY_INT, 50);
			}
		}
		break;
