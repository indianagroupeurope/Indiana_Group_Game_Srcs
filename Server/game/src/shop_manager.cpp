...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	if(pkChr->IsDead())
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("olusun_biraderim"));
		return false;
	}

#ifdef NEW_PET_SYSTEM
	if (pkChr->GetNewPetSystem()->IsActivePet())
	{
		pkChr->ChatPacket( CHAT_TYPE_INFO, LC_TEXT("not open shop window when new pet"));
		return false;
	}
#endif

	//PREVENT_TRADE_WINDOW
	if (pkChr->IsOpenSafebox() || pkChr->GetExchange() || pkChr->GetMyShop() || pkChr->IsCubeOpen() || pkChr->GetOfflineShopOwner())
	{
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ruha_bagli_itemi_satamazsin"));
		return;
	}
