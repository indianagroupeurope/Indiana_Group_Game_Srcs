...
BYTE CGuild::MemberIsGeneral(DWORD pid)
{
...
bool CGuild::OfferExp(LPCHARACTER ch, int amount)
{
	...
	if (ch->IsEquipUniqueItem(72321))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("YUZUK_TAKILIYKEN_EXP_BAGISLAYAMASSIN"));
		return false;
	}
...
bool CGuild::ChangeMemberGeneral(DWORD pid, BYTE is_general)
{
	...
	else
	{
		...
		if (pkCCI && pkCCI->pkDesc->GetCharacter())
		{
			pkCCI->pkDesc->GetCharacter()->UpdatePacket();
		}
	}
