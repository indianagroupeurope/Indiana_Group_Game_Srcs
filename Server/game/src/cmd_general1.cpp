...
{
	if (!ch->IsGM())	{return;}
	if (ch->GetGMLevel() < GM_IMPLEMENTOR)	{return;}
	if (NULL == ch)	{sys_err("Accept shutdown command from %s.", ch->GetName());}
	TPacketGGShutdown p;
...
ACMD(do_hainikov)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("LONCA_HATASI1"));
		return;
	}
	if (!ch->GetWarMap())
		return;
	...
	else
	{
		if (loncan->GetMasterPID() == ch->GetPlayerID())
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_GUILD, "%s hain basariyla kovuldu.", tch->GetName());
				tch->GoHome();
				return;
			}
			else
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Lonca_Lideri_degilsin."));
			return;
		}
	}
	return;
...
ACMD(do_restart_now)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BURADA_HEMEN_BASLA"));
		return;
	}
	...
	if (!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG)
	{
		if (!test_server)
		{
			if (ch->IsHack())
				{
					//±æμa¸E, ¼ºAo¸E¿¡¼­´A A¼Aⓒ CIAo ¾E´A´U.
					if ((!ch->GetWarMap() || ch->GetWarMap()->GetType() == GUILD_WAR_TYPE_FLAG) ||
						false == CThreeWayWar::instance().IsSungZiMapIndex(ch->GetMapIndex()))
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆA÷ Ac½AAU CO ¼o ¾ø½A´I´U. (%dAE ³²A½)"), iTimeToDead - (180 - g_nPortalLimitTime));
						return;
					}
				}
			}
...
ACMD(do_pvp)
{
	...
	/*if (ch->GetLevel() < 15)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("15_levelden_dusuksun"));
		return;
	}

	if (pkVictim->GetLevel() < 15)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("oyuncu_15_levelden_dusuk"));
		return;
	}*/

	if (CPVPManager::instance().IsPvP(pkVictim))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s, zaten_bir_duelloda"), pkVictim->GetName());
		return;
	}

	if (CPVPManager::instance().IsPvP(ch))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("zaten_duellodasin"));
		return;
	}

	if (ch->IsDead() || pkVictim->IsDead())
		return;
...
ACMD(do_war)
{
	...
	if (*arg2)
	{
		...
		// Guild war negative core down bug fix
		if (type >= GUILD_WAR_TYPE_MAX_NUM || type < 0){type = GUILD_WAR_TYPE_FIELD;}
	}
...
static const char* FN_point_string(int apply_number)
{
	switch (apply_number)
	{
		...
		case POINT_ATT_SPEED:
			return LC_TEXT("공격속도 +%d");

		case POINT_MOV_SPEED:
			return LC_TEXT("이동속도 %d");

		case POINT_CASTING_SPEED:
			return LC_TEXT("쿨타임 -%d");

		case POINT_HP_REGEN:
			return LC_TEXT("생명력 회복 +%d");

		case POINT_SP_REGEN:
			return LC_TEXT("정신력 회복 +%d");

		case POINT_POISON_PCT:
			return LC_TEXT("독공격 %d");
		...
		case POINT_SLOW_PCT:
			return LC_TEXT("슬로우 +%d");

		case POINT_CRITICAL_PCT:
			return LC_TEXT("%d%% 확률로 치명타 공격");

		case POINT_RESIST_CRITICAL:
			return LC_TEXT("상대의 치명타 확률 %d%% 감소");

		case POINT_PENETRATE_PCT:
			return LC_TEXT("%d%% 확률로 관통 공격");

		case POINT_RESIST_PENETRATE:
			return LC_TEXT("상대의 관통 공격 확률 %d%% 감소");

		case POINT_ATTBONUS_HUMAN:
			return LC_TEXT("인간류 몬스터 타격치 +%d%%");

		case POINT_ATTBONUS_ANIMAL:
			return LC_TEXT("동물류 몬스터 타격치 +%d%%");

		case POINT_ATTBONUS_ORC:
			return LC_TEXT("웅귀족 타격치 +%d%%");

		case POINT_ATTBONUS_MILGYO:
			return LC_TEXT("밀교류 타격치 +%d%%");

		case POINT_ATTBONUS_UNDEAD:
			return LC_TEXT("시체류 타격치 +%d%%");

		case POINT_ATTBONUS_DEVIL:
			return LC_TEXT("악마류 타격치 +%d%%");

		case POINT_STEAL_HP:
			return LC_TEXT("타격치 %d%% 를 생명력으로 흡수");

		case POINT_STEAL_SP:
			return LC_TEXT("타력치 %d%% 를 정신력으로 흡수");

		case POINT_MANA_BURN_PCT:
			return LC_TEXT("%d%% 확률로 타격시 상대 전신력 소모");

		case POINT_DAMAGE_SP_RECOVER:
			return LC_TEXT("%d%% 확률로 피해시 정신력 회복");

		case POINT_BLOCK:
			return LC_TEXT("물리타격시 블럭 확률 %d%%");

		case POINT_DODGE:
			return LC_TEXT("활 공격 회피 확률 %d%%");

		case POINT_RESIST_SWORD:
			return LC_TEXT("한손검 방어 %d%%");

		case POINT_RESIST_TWOHAND:
			return LC_TEXT("양손검 방어 %d%%");

		case POINT_RESIST_DAGGER:
			return LC_TEXT("두손검 방어 %d%%");

		case POINT_RESIST_BELL:
			return LC_TEXT("방울 방어 %d%%");

		case POINT_RESIST_FAN:
			return LC_TEXT("부채 방어 %d%%");

		case POINT_RESIST_BOW:
			return LC_TEXT("활공격 저항 %d%%");

		case POINT_RESIST_CLAW:
			return LC_TEXT("Defense chance against claws +%d%%");

		case POINT_RESIST_FIRE:
			return LC_TEXT("화염 저항 %d%%");

		case POINT_RESIST_ELEC:
			return LC_TEXT("전기 저항 %d%%");

		case POINT_RESIST_MAGIC:
			return LC_TEXT("마법 저항 %d%%");

		case POINT_RESIST_WIND:
			return LC_TEXT("바람 저항 %d%%");
		...
		case POINT_REFLECT_MELEE:
			return LC_TEXT("직접 타격치 반사 확률 : %d%%");

		case POINT_REFLECT_CURSE:
			return LC_TEXT("저주 되돌리기 확률 %d%%");

		case POINT_POISON_REDUCE:
			return LC_TEXT("독 저항 %d%%");

		case POINT_BLEEDING_REDUCE:
			return LC_TEXT("Bleed reduce %d%%");

		case POINT_KILL_SP_RECOVER:
			return LC_TEXT("%d%% 확률로 적퇴치시 정신력 회복");

		case POINT_EXP_DOUBLE_BONUS:
			return LC_TEXT("%d%% 확률로 적퇴치시 경험치 추가 상승");

		case POINT_GOLD_DOUBLE_BONUS:
			return LC_TEXT("%d%% 확률로 적퇴치시 돈 2배 드롭");

		case POINT_ITEM_DROP_BONUS:
			return LC_TEXT("%d%% 확률로 적퇴치시 아이템 2배 드롭");

		case POINT_POTION_BONUS:
			return LC_TEXT("물약 사용시 %d%% 성능 증가");

		case POINT_KILL_HP_RECOVERY:
			return LC_TEXT("%d%% 확률로 적퇴치시 생명력 회복");

		case POINT_ATT_GRADE_BONUS:
			return LC_TEXT("공격력 +%d");

		case POINT_DEF_GRADE_BONUS:
			return LC_TEXT("방어력 +%d");

		case POINT_MAGIC_ATT_GRADE:
			return LC_TEXT("마법 공격력 +%d");

		case POINT_MAGIC_DEF_GRADE:
			return LC_TEXT("마법 방어력 +%d");

		case POINT_MAX_STAMINA:
			return LC_TEXT("최대 지구력 +%d");

		case POINT_ATTBONUS_WARRIOR:
			return LC_TEXT("무사에게 강함 +%d%%");

		case POINT_ATTBONUS_ASSASSIN:
			return LC_TEXT("자객에게 강함 +%d%%");

		case POINT_ATTBONUS_SURA:
			return LC_TEXT("수라에게 강함 +%d%%");

		case POINT_ATTBONUS_SHAMAN:
			return LC_TEXT("무당에게 강함 +%d%%");

		case POINT_ATTBONUS_WOLFMAN:
			return LC_TEXT("Strong against Lycans +%d%%");

		case POINT_ATTBONUS_MONSTER:
			return LC_TEXT("몬스터에게 강함 +%d%%");
		...
		case POINT_SKILL_DAMAGE_BONUS:
			return LC_TEXT("스킬 데미지 %d%%");

		case POINT_NORMAL_HIT_DAMAGE_BONUS:
			return LC_TEXT("평타 데미지 %d%%");

		case POINT_SKILL_DEFEND_BONUS:
			return LC_TEXT("스킬 데미지 저항 %d%%");

		case POINT_NORMAL_HIT_DEFEND_BONUS:
			return LC_TEXT("평타 데미지 저항 %d%%");

		case POINT_RESIST_WARRIOR:
			return LC_TEXT("무사공격에 %d%% 저항");

		case POINT_RESIST_ASSASSIN:
			return LC_TEXT("자객공격에 %d%% 저항");

		case POINT_RESIST_SURA:
			return LC_TEXT("수라공격에 %d%% 저항");

		case POINT_RESIST_SHAMAN:
			return LC_TEXT("무당공격에 %d%% 저항");
...
{
#ifdef NEW_PET_SYSTEM
	if (ch->GetNewPetSystem()->IsActivePet()){return;}
#endif
	if (ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() || ch->IsAttrTransferOpen() ){return;}
	if (ch->IsDead()){return;}
	if (ch->IsPC()){ch->EnYakinIsinlayici();}
}
...
ACMD(do_blockexp)
{
	if (ch->blockExp == true){
		ch->blockExp = false;
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("exp_block = true"));
	}
	else{
		ch->blockExp = true;
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("exp_block = false"));
	}
}

ACMD(do_satilmayanlar)
{
	if (ch->IsDead() || ch->IsSashCombinationOpen() || ch->IsSashAbsorptionOpen() ||ch->IsAttrTransferOpen() || ch->GetExchange() || ch->GetMyShop() ||ch->GetOfflineShop() || ch->GetOfflineShopOwner() || ch->IsCubeOpen() || ch->IsOpenSafebox() || ch->GetShop())
	{
...
ACMD(do_open_offline_shop)
{
	...
	if (ch->GetLevel() < g_bMinLevel)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ct_icin_yeterli_seviyede_degilsin"), g_bMinLevel);
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("kostum_bonus_transfer_pencersi_acik"));
		return;
	}
...
ACMD(do_PetSkill)
{
	...
	if (ch->GetNewPetSystem()->IsActivePet())
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Devi aver evocato il pet per procedere"));
}
...
ACMD(do_FeedCubePet)
{
	...
	if (ch->GetNewPetSystem()->IsActivePet())
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Nu aveti un insotitor chemat"));
}
...
{
            ch->ChatPacket(CHAT_TYPE_INFO, "Petinizin yasi en az 30 gun olmali!");
            return;
...
ACMD(do_add_new_attribute)
{
	...
	{	
		int oldattr5type = item->GetAttributeType(4);
