...
EVENTFUNC(bleed_event)
{
	...
	int dam = ch->GetMaxHP() * GetBleedDamageRate(ch) / 3000;
...
void CHARACTER::AttackedByPoison(LPCHARACTER pkAttacker)
{
	...
	if (m_pkBleedEvent) // zehirliyken kanama fix
		return;

	if (m_bHasBleeded && !IsPC()) // zehirliyken kanama fix
		return;
...
void CHARACTER::AttackedByBleeding(LPCHARACTER pkAttacker)
{
	...
	if (m_pkPoisonEvent) // kanamaliyken zehir fix
		return;

	if (m_bHasPoisoned && !IsPC()) // kanamaliyken zehir fix
		return;
