	...
	public:
		...
		void operator() (LPDESC d)
		{
			if (d->GetCharacter())
			{
				...
				int usercount = DESC_MANAGER::instance().GetUserCountNew();
				int total = usercount + quest::CQuestManager::instance().GetEventFlag("fake_online_count");
				d->ChatPacket(CHAT_TYPE_COMMAND, "UserCount %d", total);
			}
		}
};

struct FSendOfflineShopCount
{
	char buf[128];
	FSendOfflineShopCount(DWORD total)
	{
		snprintf(buf, sizeof(buf), "OfflineShopCount %d", total);
	}
	...
	void operator() (LPDESC d)
	{
		if (d->GetCharacter())
		{
			d->ChatPacket(CHAT_TYPE_COMMAND, buf);
		}
	}
};
...
void DESC_MANAGER::UpdateOfflineShopCount()
{
	const DESC_SET & c_ref_set = GetClientSet();
	std::auto_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM player.offline_shop_npc"));
	MYSQL_ROW row = mysql_fetch_row(pmsg->Get()->pSQLResult);
	DWORD total;
	str_to_number(total, row[0]);
	FSendOfflineShopCount f(total);
	f = std::for_each(c_ref_set.begin(), c_ref_set.end(), f);
}

int DESC_MANAGER::GetUserCountNew()
{
	int iCount = P2P_MANAGER::instance().GetCountNew();
	int iTotal = m_iLocalUserCount + iCount;
	return iTotal;
}
