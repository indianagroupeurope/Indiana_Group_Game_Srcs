...
void CHARACTER::Cards_open(DWORD safemode)
{
	...
	if (character_cards.cards_left <= 0) 
	{
		if (GetGold() < 5000000)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("okey_yeterli_yang_yok"));
			return;
		}
		if (CountSpecifyItem(79506) < 1)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("okey_kart_seti_yok"));
			return;
		}
...
void CHARACTER::Cards_pullout()
{
	...
	if (empty_space == -1)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("okey_yeterli_envanter_yeri_yok"));
		return;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("okey_masada_yeterli_yer_yok"));
		return;
	}
	RandomizeCards();
	SendUpdatedInformations();
...
void CHARACTER::GetGlobalRank(char * buffer, size_t buflen)
{
	...
	*buffer = '\0';
	while (NULL != (row = mysql_fetch_row(pMsg->Get()->pSQLResult)))
	{
		...
		len2 = snprintf(buffer + len, buflen - len, "%d. Sira - %s - %d puan.", 
				pos,
				row[0],
				points);
		++pos;
