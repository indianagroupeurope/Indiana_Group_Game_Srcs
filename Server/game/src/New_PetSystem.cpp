...
void CNewPetActor::ItemCubeFeed(int type)
{
	for (int i = 0; i < 9; ++i) {
		if (m_dwpetslotitem[i] != -1) {
			...
			{
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ruha_bagli"));
				return;
			}
			if(type == 1)
			{
			...
			else if(type == 3)
            {
                if(itemxp->GetType() == 1 || itemxp->GetType() == 2)
                {
                    if (GetNextExpFromItem() - GetExpI() > 0)
                    {
                        ...
                        m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Petiniz Doydu Daha fazla item-exp veremezsiniz.");
                }
            }
		}
	}
...
bool CNewPetActor::IncreasePetSkill(int skill) {
	...
	for (int i = 0; i < 3; ++i) { //Itero gli slot per cercare la skill
		if (m_dwskillslot[i] == skill) {  //Se trova la skill o la aumenta oppure e' gi?max
			if (m_dwskill[i] < 20) {
				m_dwskill[i] += 1;
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Skillul ta a atins nivelul:%d"), m_dwskill[i]);
				m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetSkill %d %d %d", i, m_dwskillslot[i], m_dwskill[i]);
				return true;
			}
			else {
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Skillul tau a atins deja nivelul maxim!"));
				return false;
			}
		}
	}
	...
	for (int i = 0; i < 3; ++i) {
		if (m_dwskillslot[i] == 0 || m_dwskillslot[i] == -1) { 
			...
			m_dwskill[i] = 1;
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Animalul tau de companie a invatat un nou skill!"));
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetSkill %d %d %d", i, m_dwskillslot[i], m_dwskill[i]);
			return true;
		}
	}
	...
	*/
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Animalul tau de companie nu este capabil sa invete alte abilitati!"));
	return false;
}
...
bool CNewPetActor::IncreasePetEvolution() {
	if (m_dwevolution < 3) {
		...
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Animalul tau de companie a atins evolutia maxima!"));
		return false;
	}
	return true;
}
...
void CNewPetActor::SetExp(DWORD exp, int mode)
{
	...
	if (exp < 0) {exp = MAX(m_dwexp - exp, 0);}
	if(mode == 0){
...
{
	// if(pSummonItem->GetAttributeValue(3)<1)
	// {
		// m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PS_PET_OLMUS"));
		// return 0;
	// }
...
//	m_pkChar->DetailLog();
	...
	char szQuery1[1024];
	snprintf(szQuery1, sizeof(szQuery1), "SELECT name,level,exp,expi,bonus0,bonus1,bonus2,skill0,skill0lv,skill1,skill1lv,skill2,skill2lv,duration,tduration,evolution,Petin_yasi FROM new_petsystem WHERE id = %d ", pSummonItem->GetID());
...
void CNewPetActor::DoPetSkill(int skillslot) {
	...
	case 10:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(0) <= 480) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Abilitatea se reincarca!%ds ramase"), 480 - (get_global_time() - m_pkOwner->GetNewPetSkillCD(0)));
			return;
		}
		if (m_pkOwner->GetHPPct() > 20) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Poti utiliza aceasta abilitate numai cu pv-ul <= 20%!"));
			return;
		}
	...
	case 17:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(1) <= 600) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Abilitatea se reincarca!%ds ramase"), 600 - (get_global_time() - m_pkOwner->GetNewPetSkillCD(1)));
			return;
		}
		m_pkOwner->SetNewPetSkillCD(1, get_global_time());
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Ai devenit nemuritor!"));
	...
	case 18:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(2) <= 480) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Abilitatea se reincarca!%ds ramase"), 480 -(get_global_time() - m_pkOwner->GetNewPetSkillCD(2)));
			return;
		}
...
int CNewPetSystem::GetYas()//for server
{
    for (TNewPetActorMap::iterator iter = m_petActorMap.begin(); iter != m_petActorMap.end(); ++iter)
    {
        CNewPetActor* petActor = iter->second;
        if (petActor != 0)
        {
            if (petActor->IsSummoned()) 
            {
                return petActor->GetPetYasi();//only CNewPetActor
            }
        }
    }
    return -1;
