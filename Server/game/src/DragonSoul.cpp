...
bool DSManager::ExtractDragonHeart(LPCHARACTER ch, LPITEM pItem, LPITEM pExtractor)
{
	...
	if (pItem->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_GIYILEN_ITEM"));
		return false;
	}
	...
	if (fCharge < FLT_EPSILON)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_BASARISIZ"));
		return false;
	}
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_BASARILI"));
		return true;
	}
}
...
bool DSManager::PullOut(LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	if (!IsValidCellForThisItem(pItem, DestCell))
	{
		...
		if (iEmptyCell < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_SE_DOLU"));
			return false;
		}
		else
		{
	...
	// 캐릭터의 용혼석 추출 및 추가 혹은 제거. 부산물 제공.
	{
		...
		else
		{
			...
			if (dwByProduct)
			{
				...
				if (pByProduct)
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_BASARISIZ_KAZANDIN"), pByProduct->GetName());
				else
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_BASARISIZ"));
			}
			else
...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (!ch->DragonSoul_RefineWindow_CanRefine())
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "DSS_YUKSELTME_PENCERESI_KAPALI");
		return false;
	}
	...
	{
		if (aItemPoses[i].IsEquipPosition()){return false;}
		LPITEM pItem = ch->GetItem(aItemPoses[i]);
		if (NULL != pItem){
			if (!pItem->IsDragonSoul()){
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_Y_YETERSIZ_MALZEME"));
	...
	std::set <LPITEM>::iterator it = set_items.begin();
	{
		...
		if (!m_pTable->GetRefineGradeValues(ds_type, grade_idx, need_count, fee, vec_probs)){
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_BU_YUKSELTILEMEZ"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	...
	if (ch->GetGold() < fee){
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_YETERSIZ_YANG"));
	...
	if (result_grade > grade_idx)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_YUKSELTME_BASARILI"));
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_YUKSELTME_BASARISIZ"));
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	for (int i = 0; i < DRAGON_SOUL_REFINE_GRID_SIZE; i++)
	{
		...
		if (NULL != pItem)
		{
			...
			if (!pItem->IsDragonSoul())
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_BUNESNE_SIMYA_DEGIL"));
				SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
				return false;
			}
			set_items.insert(pItem);
	...
	std::set <LPITEM>::iterator it = set_items.begin(); 
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DDS_BU_TAS_YUKSELTILEMEZ"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;
		}
	}
	...
	float sum = 0.f;
	if (-1 == (result_step = Gamble(vec_probs))){sys_err ("Gamble failed. See RefineStepTables' probabilities");return false;}
	...
	for (std::set <LPITEM>::iterator it = set_items.begin(); it != set_items.end(); it++){
		LPITEM pItem = *it;
		int n = pItem->GetCount();
		if (left_count > n){pItem->RemoveFromCharacter();/**/M2_DESTROY_ITEM(pItem);/**/left_count -= n;}
	...
	if (result_step > step_idx){
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_ARITMA_BASARILI"));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_SUCCEED, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return true;}
	else{
		char buf[128];
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_ARITMA_BASARISIZ"));
		SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL, TItemPos (pResultItem->GetWindow(), pResultItem->GetCell()));
		return false;}
}
...
bool DSManager::DoRefineStrength(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (NULL == aItemPoses){return false;}
	if (!ch->DragonSoul_RefineWindow_CanRefine()){
		sys_err ("%s do not activate DragonSoulRefineWindow. But how can he come here?", ch->GetName());
		ch->ChatPacket(CHAT_TYPE_INFO, "DSS_HATA");
		return false;}
	...
	if (set_items.size() == 0){return false;}
	int fee;
	...
	for (std::set <LPITEM>::iterator it = set_items.begin(); it != set_items.end(); it++){
		...
		if (pItem->IsEquipped()){return false;}
		if (pItem->IsDragonSoul()){
			if (pDragonSoul != NULL){SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_TOO_MUCH_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
				return false;}
			pDragonSoul = pItem;}
			...
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_YETERSIZ_MALZEME"));
			SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_INVALID_MATERIAL, TItemPos(pItem->GetWindow(), pItem->GetCell()));
			return false;}
	}
	...
	if (!pDragonSoul || !pRefineStone){SendRefineResultPacket(ch, DS_SUB_HEADER_REFINE_FAIL_NOT_ENOUGH_MATERIAL, NPOS);return false;}
	if (NULL != pDragonSoul)
	{
	...
	if (fnumber(0.f, 100.f) <= fProb){
		...
		if (NULL == pResult){sys_err ("INVALID DRAGON SOUL(%d)", MakeDragonSoulVnum(bType, bGrade, bStep, bStrength + 1));return false;}
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("DSS_GUCLENDIRME_BASARISIZ"));
		pDragonSoul->SetCount(pDragonSoul->GetCount() - 1);
		pRefineStone->SetCount(pRefineStone->GetCount() - 1);
		if (NULL != pResult)
			ch->AutoGiveItem(pResult, true);
