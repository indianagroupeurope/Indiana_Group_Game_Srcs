...
bool CHARACTER::ExchangeStart(LPCHARACTER victim)
{
	...
	if ( GetOfflineShopOwner() || IsOpenSafebox() || GetShopOwner() || GetMyShop() || IsCubeOpen() || IsAttrTransferOpen())
	{
	...
	if ( victim->GetOfflineShopOwner() || victim->IsOpenSafebox() || victim->GetShopOwner() || victim->GetMyShop() || victim->IsCubeOpen() || victim->IsAttrTransferOpen() )
	{
...
bool CExchange::AddItem(TItemPos item_pos, BYTE display_pos)
{
	...
	{
		m_pOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ruha_bagli_nesneyi_takasta_kullanamazsin"));
		return false;
	}
