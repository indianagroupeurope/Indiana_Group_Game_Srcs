...
namespace MeleyLair
{
	...
	void CMgrMap::SetDungeonStep(BYTE bStep)
	{
		...
		else if (bStep == 4)
		{
			...
			snprintf(szBuf, sizeof(szBuf), LC_TEXT(ENGLISH, "Hai %d secondo(i) per distruggere le statue, sbrigati!"), TIME_LIMIT_TO_KILL_STATUE);
			#else
			snprintf(szBuf, sizeof(szBuf), LC_TEXT("Hai %d secondo(i) per distruggere le statue, sbrigati!"), TIME_LIMIT_TO_KILL_STATUE);
			#endif
			FNotice f(szBuf);
	...
	void CMgrMap::StartDungeon(LPCHARACTER pkChar)
	{
		...
		#ifdef __MULTI_LANGUAGE_SYSTEM__
		snprintf(szBuf1, sizeof(szBuf1), LC_TEXT(ENGLISH, "<Gilda> Il combattimento contro %s inizia."), pkBoss->GetName());
		snprintf(szBuf2, sizeof(szBuf2), LC_TEXT(ENGLISH, "<Gilda> Le porte del rifugio sono ora aperte. Il limite di tempo è %d ora(e)."), (int)(TIME_LIMIT_DUNGEON / 3600));
		#else
		snprintf(szBuf1, sizeof(szBuf1), LC_TEXT("<Gilda> Il combattimento contro %s inizia."), pkBoss->GetName());
		snprintf(szBuf2, sizeof(szBuf2), LC_TEXT("<Gilda> Le porte del rifugio sono ora aperte. Il limite di tempo è %d ora(e)."), (int)(TIME_LIMIT_DUNGEON / 3600));
		#endif
		pkGuild->Chat(szBuf1);
		pkGuild->Chat(szBuf2);
		...
		if (pkGuild)
		{
			...
			if (bGiveBack)
			{
				if (bSuccess)
				{
					...
					if (iM > 0)
						#ifdef __MULTI_LANGUAGE_SYSTEM__
						time.append(boost::lexical_cast<std::string>(iM) + LC_TEXT(ENGLISH, " minuto(i)"));
						#else
						time.append(boost::lexical_cast<std::string>(iM) + LC_TEXT(" minuto(i)"));
						#endif
					...
					if (iS > 0)
					{
						...
						#ifdef __MULTI_LANGUAGE_SYSTEM__
						time.append(boost::lexical_cast<std::string>(iS) + LC_TEXT(ENGLISH, " secondo(i)"));
						#else
						time.append(boost::lexical_cast<std::string>(iS) + LC_TEXT(" secondo(i)"));
						#endif
					}
					...
					#ifdef __MULTI_LANGUAGE_SYSTEM__
					snprintf(szBuf1, sizeof(szBuf1), LC_TEXT(ENGLISH, "<Gilda> Complimenti, hai sconfitto Meley, la Regina dei Draghi."));
					snprintf(szBuf2, sizeof(szBuf2), LC_TEXT(ENGLISH, "<Gilda> Tempo necessario: %s."), time.c_str());
					#else
					snprintf(szBuf1, sizeof(szBuf1), LC_TEXT("<Gilda> Complimenti, hai sconfitto Meley, la Regina dei Draghi."));
					snprintf(szBuf2, sizeof(szBuf2), LC_TEXT("<Gilda> Tempo necessario: %s."), time.c_str());
					#endif
				}
				else
				{
					...
					iReturn = (int)(LADDER_POINTS_COST);
					#ifdef __MULTI_LANGUAGE_SYSTEM__
					snprintf(szBuf1, sizeof(szBuf1), LC_TEXT(ENGLISH, "<Gilda> Il dungeon non può essere avviato, si prega di contattare il nostro staff."));
					snprintf(szBuf2, sizeof(szBuf2), LC_TEXT(ENGLISH, "<Guild> %d i punti gilda saranno restituiti."), iReturn);
					#else
					snprintf(szBuf1, sizeof(szBuf1), LC_TEXT("<Gilda> Il dungeon non può essere avviato, si prega di contattare il nostro staff."));
					snprintf(szBuf2, sizeof(szBuf2), LC_TEXT("<Guild> %d i punti gilda saranno restituiti."), iReturn);
					#endif
				}
			}
			else
			{
				...
				#ifdef __MULTI_LANGUAGE_SYSTEM__
				snprintf(szBuf1, sizeof(szBuf1), LC_TEXT(ENGLISH, "<Gilda> Il limite di tempo per combattere è terminato."), pkBoss->GetName());
				snprintf(szBuf2, sizeof(szBuf2), ".");
				#else
				snprintf(szBuf1, sizeof(szBuf1), LC_TEXT("<Gilda> Il limite di tempo per combattere è terminato."), pkBoss->GetName());
				snprintf(szBuf2, sizeof(szBuf2), ".");
				#endif
			}
			...
			if (pkSectreeMap)
			{
				...
				#ifdef __MULTI_LANGUAGE_SYSTEM__
				if (iWarpTime == 60)
					snprintf(szBuf, sizeof(szBuf), LC_TEXT(ENGLISH, "<Gilda> Per la tua sicurezza, sarai teletrasportato fuori dal rifugio in %d secondo/i."), iWarpTime);
				else
					snprintf(szBuf, sizeof(szBuf), LC_TEXT(ENGLISH, "<Gilda> Tutti i partecipanti verranno teletrasportati in %d secondi."), iWarpTime);
				#else
				if (iWarpTime == 60)
					snprintf(szBuf, sizeof(szBuf), LC_TEXT("<Gilda> Per la tua sicurezza, sarai teletrasportato fuori dal rifugio in %d secondo/i."), iWarpTime);
				else
					snprintf(szBuf, sizeof(szBuf), LC_TEXT("<Gilda> Tutti i partecipanti verranno teletrasportati in %d secondi."), iWarpTime);
				#endif
	...
	void CMgr::OpenRanking(LPCHARACTER pkChar)
	{
		...
		std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT * FROM log.meley_dungeon%s ORDER BY time ASC, partecipants ASC, date ASC LIMIT 5;", get_table_postfix()));
