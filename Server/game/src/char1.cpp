...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ruha_bagli_itemi_satamazsin"));
				return;
			}
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC)
	{
...
#ifdef NEW_PET_SYSTEM
		if (IsPC() == true || IsNewPet() == true || (IsOfflineShopNPC() && GetRaceNum() == 30000))
#else
		if (IsPC() == true || (IsOfflineShopNPC() && GetRaceNum() == 30000))
#endif
		{
		...
		addPacket.lWeaponRarity = weapon ? weapon->GetAttributeValue(6) : 0;
		...
		show_all_info:
			...
			{	
				...
				if (pGuild->GetMasterPID() == GetPlayerID())
					addPacket.dwGuildAuth = 3;
				else if (pGuild && pGuild->MemberIsGeneral(GetPlayerID()) == true)
					addPacket.dwGuildAuth = 2;
				else
					addPacket.dwGuildAuth = 1;
			}
			else
			{
				addPacket.dwGuildID = 0;
	...
	//isaekledi.
	//if (IsPC() && IsOfflineShopNPC() && GetRaceNum() == 30000)
...
void CHARACTER::UpdatePacket()
{
	...
	LPITEM weapon = GetWear(WEAR_WEAPON);
	pack.lWeaponRarity = weapon ? weapon->GetAttributeValue(6) : 0;
...
void CHARACTER::MainCharacterPacket()
{
	...
	if (!bgmInfo.name.empty())
	{
		...
		// if (m_stMobile.length()) {ChatPacket(CHAT_TYPE_COMMAND, "sms");}
	}
	// END_OF_SUPPORT_BGM
	else
	{
...
void CHARACTER::CalculateMoveDuration()
{
	...
	if (IsNPC())
		sys_log(1, "%s: GOTO(char.cpp): distance %f, spd %u, duration %u, motion speed %f pos %d %d -> %d %d",
		...
		case POINT_EXP:
			{
				...
				{	
					...
					if (IsEquipUniqueItem(ITEM_SIRLAR_YUZUGU))
						return;
...
#define __OFFLINE_SHOP_VALUE_MAX 2000000000
	...
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT money2 FROM player.player WHERE id = %u", GetPlayerID()));
    ...
    if (__OFFLINE_SHOP_VALUE_MAX <= sTotalMoneyVegas){ChatPacket(CHAT_TYPE_INFO, LC_TEXT("20억 냥을 초과하여 상점을 열수가 없습니다")); return;}
#else    
	if (!LC_IsBrazil()){
		DWORD nTotalMoney = 0;
...

	{
		...
		if (bResult){
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("At first you will get your items after you can open offline shop again."));
	...
	for (BYTE i = 0; i < bItemCount; ++i){
		...
		if (pkItem){
			...
			if (pkItem->IsBind() || pkItem->IsUntilBind()){ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ruha_bagli_nesne"));return;}
			...
			int iUpdateLen = snprintf(szValues, sizeof(szValues), "%u,%u,%d,%u,%u,%u,%u,%d", pkItem->GetID(), GetPlayerID(), (pTable + i)->display_pos, pkItem->GetCount(), (pTable + i)->price, (pTable + i)->price_cheque, pkItem->GetVnum(), pkItem->GetTransmutation());
	...
	char szQuery2[512];
	if (GetGuild())
		snprintf(szQuery2, sizeof(szQuery2), "INSERT INTO %soffline_shop_npc(owner_id, sign, name, time, x, y, z, mapIndex, channel, level, align, guild) VALUES(%u, '%s', '%s', %d, %ld, %ld, %ld, %ld, %d, %d, %d, '%s')", get_table_postfix(), GetPlayerID(), c_pszSign, GetName(), iTime, GetX(), GetY(), GetZ(), GetMapIndex(), g_bChannel, GetLevel(), GetAlignment(), GetGuild()->GetName());
	else
		snprintf(szQuery2, sizeof(szQuery2), "INSERT INTO %soffline_shop_npc(owner_id, sign, name, time, x, y, z, mapIndex, channel, level, align, guild) VALUES(%u, '%s', '%s', %d, %ld, %ld, %ld, %ld, %d, %d, %d, '%s')", get_table_postfix(), GetPlayerID(), c_pszSign, GetName(), iTime, GetX(), GetY(), GetZ(), GetMapIndex(), g_bChannel, GetLevel(), GetAlignment(), 0);
	...
	if (IsPC())
	{
		if (!CTargetManager::instance().GetTargetInfo(pkChrCauser->GetPlayerID(), TARGET_TYPE_VID, GetVID()))
		{
			if (GetMyShop())
			{
				...
				if (pkChrCauser == this)
				{
					if ((GetExchange() || IsOpenSafebox() || GetShopOwner()) || IsCubeOpen() || IsAttrTransferOpen() || GetOfflineShopOwner())
					{
				...
				{
					if ((pkChrCauser->GetExchange() || pkChrCauser->IsOpenSafebox() || pkChrCauser->GetMyShop() || pkChrCauser->GetShopOwner()) || pkChrCauser->IsCubeOpen() || pkChrCauser->IsAttrTransferOpen() || pkChrCauser->GetOfflineShopOwner() )
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("다른 거래중(창고,교환,상점)에는 개인상점을 사용할 수 없습니다."));
						return;
					}

					if ((GetExchange() || IsOpenSafebox() || IsCubeOpen() || IsAttrTransferOpen() || GetOfflineShopOwner()))
					{
...
{
#ifdef __ANTI_RESIST_MAGIC_BONUS__
	const int METIN_STONE_NUM = 18;
#else
	const int METIN_STONE_NUM = 16;
#endif
...
bool CHARACTER::IsHack(bool bSendMsg, bool bCheckShopOwner, int limittime)
{
	...
	{
		if (GetExchange() || GetMyShop() || IsOpenSafebox() || IsCubeOpen() || IsAttrTransferOpen())
		{
			if (bSendMsg)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("거래창,창고 등을 연 상태에서는 다른곳으로 이동,종료 할수 없습니다"));
...
DWORD CHARACTER::yeniBirim_miktar()
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT yeniBirim FROM account.account WHERE id = '%d';", GetDesc()->GetAccountTable().id));
	if (pMsg->Get()->uiNumRows == 0)
		return 0;
	MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
	DWORD yeniBirim = 0;
	str_to_number(yeniBirim, row[0]);
	return yeniBirim;
}

void CHARACTER::yeniBirim_guncelle(DWORD amount)
{
	DBManager::instance().DirectQuery("UPDATE account.account SET yeniBirim = '%d' WHERE id = '%d';", amount, GetDesc()->GetAccountTable().id);
}

void CHARACTER::EnYakinIsinlayici()
{
	if (GetMapIndex() == 1)
	{
		DWORD x = 465158;
