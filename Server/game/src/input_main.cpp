...
void CInputMain::BraveRequestPetName(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[PetSystem] Ai nevoie de 100.000 yang"));
	}
	...
	if (ch->CountSpecifyItem(vid) > 0 && check_name(p->petname) != 0)
	{
		...
		char szQuery1[1024];
		int tmpduryas = 1440;
		snprintf(szQuery1, sizeof(szQuery1), "INSERT INTO new_petsystem VALUES(%d,'%s', 1, 0, 0, 0, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)", item->GetID(), p->petname, number(1, 23), number(1, 23), number(1, 23), tmpskill[0], 0, tmpskill[1], 0, tmpskill[2], 0, tmpdur, tmpdur, tmpduryas);
		std::auto_ptr<SQLMsg> pmsg2(DBManager::instance().DirectQuery(szQuery1));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[PetSystem]Eroare in setarea numelui"));
	}
}
#endif
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (ch->IncreaseChatCounter() >= 10)
	{
		if (ch->GetChatCounter() == 10)	{ch->GetDesc()->DelayedDisconnect(5);}
		return iExtraLen;
	}
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BAGIRMA_LEVELI"), SHOUT_LIMIT_LEVEL);
			return (iExtraLen);
		}
		...
		const char* kingdoms[4] = {"|cFF47DA00|H|h[GameMaster]|cFFA7FFD4|H|h","|cFFff0000|H|h[S]|cFFA7FFD4|H|h","|cFFFFFF00|H|h[C]|cFFA7FFD4|H|h","|cFF0080FF|H|h[J]|cFFA7FFD4|H|h"};
		...
		if (gSpecialShout == 1)
		{
			if (ch->GetGMLevel() != GM_PLAYER)
			{
				...
				else
				{
					...
					{
						int len_global = snprintf(chatbuf_global, sizeof(chatbuf_global), "%s %s : %s",kingdoms[ch->GetEmpire()], ch->GetName(), buf);
					}
					else if ((MasterColorEmpire == 1) and (MasterLevelChat == 1))
					{
						int len_global = snprintf(chatbuf_global, sizeof(chatbuf_global), "%s [Lv %d] %s : %s",kingdoms[ch->GetEmpire()], my_level, ch->GetName(), buf);
					}
					else if ((MasterColorEmpire == 0) and (MasterLevelChat == 1))
					{
						int len_global = snprintf(chatbuf_global, sizeof(chatbuf_global), "[Lv %d] %s : %s",my_level, ch->GetName(), buf);
					}
					else
					{
	...
	/*if (pinfo->type == CHAT_TYPE_SHOUT_TRADE)
	{
		...
		if (thecore_heart->pulse - (int) ch->GetLastShoutPulse() < passes_per_sec * 1)
			return (iExtraLen);
		...
		SendShoutTrade(chatbuf_global, ch->GetEmpire());
...
int CInputMain::OfflineShop(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	switch (p->subheader)
	{
		...
		case SHOP_SUBHEADER_CG_ADD_ITEM:
		{
			...
			COfflineShopManager::instance().AddItem(ch, pTable->bDisplayPos, pTable->bPos, pTable->lPrice, pTable->lPriceCheque);
		...
		case SHOP_SUBHEADER_CG_CHANGE_PRICE:
		{
			...
			COfflineShopManager::instance().ChangePrice(ch, pTable->bDisplayPos, pTable->lPrice, pTable->lPriceCheque);
			return (sizeof(TOfflineShopItemTable));
		}
			...
			if (!ch->GetExchange())
			{
				if ((to_ch = CHARACTER_MANAGER::instance().Find(pinfo->arg1)))
				{
					...
					*/
					...
					if (ch->GetOfflineShopOwner() || ch->GetMyShop() || ch->IsOpenSafebox() || ch->GetShopOwner() || ch->IsCubeOpen() || ch->IsAttrTransferOpen())
					{
...
//	if (!test_server)	//2012.05.15 김용욱 : 테섭에서 (무적상태로) 다수 몬스터 상대로 다운되면서 공격시 콤보핵으로 죽는 문제가 있었다.
	{
		...
		if (((false == ch->IsRiding() && fDist > 100) || fDist > 160) && OXEVENT_MAP_INDEX != ch->GetMapIndex())
		{
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("once_kemer_envanterini_bosalt"));
		return;
	}
	...
	if (pkItemNew)
    {
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("UYARI_KOSTUM"));
		return;
	}
...
void CInputMain::SafeboxCheckout(LPCHARACTER ch, const char * c_pData, bool bMall)
{
	...
	for (WORD belt_index = BELT_INVENTORY_SLOT_START; belt_index < BELT_INVENTORY_SLOT_END; ++belt_index)
	{
		if (pkItem->GetType() != 3 && p->ItemPos.cell == belt_index)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("depodan_kemer_envanterine_nesne_koyamazsın"));
				return;
			}
		}   
	}
		...
		case GUILD_SUBHEADER_CG_ADD_MEMBER:
			{
				...
				if (!newmember->IsPC())	{return SubPacketLen;}
				if (!ch->IsPC())	{return SubPacketLen;}
		...
		case GUILD_SUBHEADER_CG_OFFER:
			{
				...
				else if (ch->blockExp == true)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("exp kazanimi ac"));
				}
				else
				{
					offer /= 100;
...
int CInputMain::MyShop(LPCHARACTER ch, const char * c_pData, size_t uiBytes)
{
	...
	if (ch->GetOfflineShopOwner() || ch->GetExchange() || ch->IsOpenSafebox() || ch->GetShopOwner() || ch->IsCubeOpen() || ch->IsAttrTransferOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("다른 거래중일경우 개인상점을 열수가 없습니다."));
		return (iExtraLen);
	}
...
int CInputMain::MyOfflineShop(LPCHARACTER ch, const char * c_pData, size_t uiBytes)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("´U¸￥ °A·¡AßAI°æ¿i °³AI≫oA¡A≫ ¿­¼o°¡ ¾ø½A´I´U."));
		return (iExtraLen);
	}
