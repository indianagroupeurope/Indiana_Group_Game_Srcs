...
#define ENABLE_PENETRATE_EFFECT
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC() && this->GetRaceNum() == 2493)
	{
		LogManager::instance().DragonLairRankingLog(pkKiller->GetPlayerID(), pkKiller->GetName(), pkKiller->GetEmpire(), 1);
	}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	if (pAttacker && this)
	{
		...
		{
			pAttacker->RemoveAffect(SKILL_GWIGEOM); //Silah yoksa buyulu keskinlik iptal agaaa
			return false;
		}
		
		if (pAttacker->IsAffectFlag(AFF_GEOMGYEONG) && !pAttacker->GetWear(WEAR_WEAPON))
		{
			pAttacker->RemoveAffect(SKILL_GEOMKYUNG); //Silah olmadan nereye hava acıyon agaa
			return false;
		}
	}
	...
	if (GetRaceNum() == 6420)
	{
		SendDamagePacket(pAttacker, 1, DAMAGE_TYPE_NORMAL);
		if (!cannot_dead)
		{
	...
	if(GetRaceNum() == 20399 && pAttacker->GetJob() != JOB_ASSASSIN && pAttacker->GetJob() != JOB_SHAMAN) // nemerede metin2 sadece ?aman ve ninja vurur.
	{
		SendDamagePacket(pAttacker, 0, DAMAGE_BLOCK);
		return false;
	}
...
*/
	...
	if (!to->IsEquipUniqueItem(ITEM_SIRLAR_YUZUGU))
	{
...
{
	LPITEM pkBow;
	if (!(pkBow = GetWear(WEAR_WEAPON)) || pkBow->GetProto()->bSubType != WEAPON_BOW){return 0;}
