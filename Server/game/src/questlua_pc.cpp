...
namespace quest 
{
	...
	int pc_change_sex(lua_State* L)  
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cinsiyet_degismek_icin_kostum_cikart"));
			lua_pushboolean(L, false);
			return 1;
		}
	...
	int pc_change_sex_degisemen(lua_State* L)
	{
		...
		if (CQuestManager::instance().GetCurrentCharacterPtr()->GetWear(WEAR_COSTUME_BODY))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("cinsiyetdegistirebilmekicinkostumunucikart"));
			lua_pushboolean(L, false);
			return 1;
		}
		return 1;
	...
	int pc_offline_shop_engel(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT COUNT(*) FROM %soffline_shop_npc WHERE owner_id = %u", get_table_postfix(), ch->GetPlayerID()));
	...
	int pc_offline_shop_engel_isimizi(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		...
		if (bResult)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("offflineshopunacikken_bunuyapamam_kardes_anladinmi"));
		}
	...
	int pc_get_yenibirim(lua_State * L)
	{ 
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->yeniBirim_miktar());
		return 1;
	}
...
teleport_area:
	...
	int _block_exp(lua_State* L)  // block exp
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->blockExp = true;
		return 0;
	...
	int _unblock_exp(lua_State* L)  // block exp
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->blockExp = false;
		return 0;
	...
	void RegisterPCFunctionTable()
	{
		...
		{
			{ "block_exp",		_block_exp		}, // block exp
			{ "unblock_exp",	_unblock_exp	}, // block exp
			...
			{ "offline_shop_engel",	pc_offline_shop_engel	},
			{ "offline_shop_engel_isimizi",	pc_offline_shop_engel_isimizi	},
			{ "gethp",			pc_get_hp		},
			{ "get_hp",			pc_get_hp		},
...
#endif
			{ "pc_get_yenibirim",	pc_get_yenibirim	},
#ifdef __CHANGELOOK_SYSTEM__
