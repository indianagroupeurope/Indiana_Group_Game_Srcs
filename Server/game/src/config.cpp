...
bool MaskesizDuygular = false;
bool KuresizYansitma = false;
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("lycan_map")
		{
			...
			if (1 == flag)
				g_bEnableNewKingdomForWolfman = true;
			else
				g_bEnableNewKingdomForWolfman = false;

			fprintf(stderr, "LYCAN_MAP: %d\n", flag);
			continue;
		}

		TOKEN("maskesiz_duygular")
		{
			BYTE flag = 0;
			str_to_number(flag, value_string);

			if (1 == flag)
				MaskesizDuygular = true;
			else
				MaskesizDuygular = false;

			fprintf(stderr, "MASKESIZ_DUYGULAR: %d\n", flag);
			continue;
		}

		TOKEN("kuresiz_yansitma")
		{
			BYTE flag = 0;
			str_to_number(flag, value_string);

			if (1 == flag)
				KuresizYansitma = true;
			else
				KuresizYansitma = false;

			fprintf(stderr, "KURESIZ_YANSITMA: %d\n", flag);
			continue;
		}

		TOKEN("tas_basma_orani")
		{
			str_to_number(TasBasmaOrani, value_string);

			fprintf(stderr, "TAS_BASMA_ORANI: %d\n", TasBasmaOrani);
		}
