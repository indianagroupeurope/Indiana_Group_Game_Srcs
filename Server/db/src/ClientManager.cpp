...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	if (pi->account_index == 0)
	{
		...
		#endif
		snprintf(szQuery, sizeof(szQuery), 
				"SELECT id, window+0, pos, count, vnum, bind, transmutation, socket0, socket1, socket2, "
...
void CClientManager::QUERY_EMPIRE_SELECT(CPeer * pkPeer, DWORD dwHandle, TEmpireSelectPacket * p)
{
	...
	sys_log(0, "EmpireSelect: %s", szQuery);
	{
		...
		if (pRes->uiNumRows)
		{
			...
			DWORD pids[3];
			UINT g_start_map[4] = {0, 1, 21, 41};
			DWORD g_start_position[4][2] = {{0, 0}, {469300, 964200}, {55700, 157900}, {969600, 278400}};
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		snprintf(szQuery, sizeof(szQuery), 
			"REPLACE INTO item%s (id, owner_id, window, pos, count, vnum, bind, transmutation, socket0, socket1, socket2, "
			"attrtype0, attrvalue0, "
			"attrtype1, attrvalue1, "
			"attrtype2, attrvalue2, "
			"attrtype3, attrvalue3, "
			...
			"attrtype5, attrvalue5, "
			"attrtype6, attrvalue6) "
			"VALUES(%u, %u, %d, %d, %u, %u, %ld, %u, %ld, %ld, %ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
			GetTablePostfix(),
			p->id,
			p->owner,
			p->window,
...
void CClientManager::MyshopPricelistUpdate(const TPacketMyshopPricelistHeader* pPacket)
{
	...
	else
	{
		...
		char szQuery[QUERY_MAX_LEN];
#ifdef ENABLE_CHEQUE_SYSTEM
		snprintf(szQuery, sizeof(szQuery), "SELECT item_vnum, price, price_cheque FROM myshop_pricelist%s WHERE owner_id=%u", GetTablePostfix(), pPacket->dwOwnerID);
#else
...
void CClientManager::MyshopPricelistRequest(CPeer* peer, DWORD dwHandle, DWORD dwPlayerID)
{
	...
	else
	{
		...
		char szQuery[QUERY_MAX_LEN];
#ifdef ENABLE_CHEQUE_SYSTEM
		snprintf(szQuery, sizeof(szQuery), "SELECT item_vnum, price, price_cheque FROM myshop_pricelist%s WHERE owner_id=%u", GetTablePostfix(), dwPlayerID);
#else
...
void CClientManager::ChangeMonarchLord(CPeer * peer, DWORD dwHandle, TPacketChangeMonarchLord* info)
{
	...
	snprintf(szQuery, sizeof(szQuery),
			...
			"(b.pid1=%u OR b.pid2=%u OR b.pid3=%u OR b.pid4=%u OR b.pid5=%u)", GetTablePostfix(), GetTablePostfix(), info->dwPID, info->bEmpire, info->dwPID, info->dwPID, info->dwPID, info->dwPID);
