...
void CItemCache::OnFlush()
{
	...
	else
	{
		...
		int iLen = snprintf(szColumns, sizeof(szColumns), "id, owner_id, window, pos, count, vnum, bind, transmutation");

		int iValueLen = snprintf(szValues, sizeof(szValues), "%u, %u, %d, %d, %u, %u, %ld, %u",
				p->id, p->owner, p->window, p->pos, p->count, p->vnum, p->bind, p->transmutation);

		int iUpdateLen = snprintf(szUpdate, sizeof(szUpdate), "owner_id=%u, window=%d, pos=%d, count=%u, vnum=%u, bind=%ld, transmutation=%u",
				p->owner, p->window, p->pos, p->count, p->vnum, p->bind, p->transmutation);
